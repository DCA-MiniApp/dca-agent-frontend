...
const arbitrumClient = createPublicClient({
  chain: arbitrum,
  transport: http(), // uses public Arbitrum RPC
});
 ...
 */
export async function createTriggerXJobForPlan(
  params: CreateTriggerXJobParams
  ...
  try {
    ...
    console.info(
      "[TRIGGERX INTEGRATION] userAddress from params:",
      userAddress
    );
    ...
    console.info(
      "[TRIGGERX INTEGRATION] scriptParams.userAddress:",
      scriptParams.userAddress
    );
    ...
    if (!validation.isValid) {
      throw new Error(
        `Invalid script parameters: ${validation.errors.join(", ")}`
      );
    }
    ...
    console.info(
      "Minimal script uploaded to IPFS:",
      uploadResult.scriptIpfsUrl
    );
    ...
    console.info(
      "[TRIGGERX INTEGRATION] jobInput created (check for userAddress in it):",
    ...
    console.info(
      "🔗 IPFS URL being passed:",
      jobInput.dynamicArgumentsScriptUrl
    );
    ...
    if (!result.success || !result.data) {
      ...
      const error: any = new Error(
        result.error || "TriggerX job creation failed"
      );
    ...
    const jobId =
      (Array.isArray((result as any)?.data?.job_id) &&
        (result as any).data.job_id[0]) ||
      (Array.isArray((result as any)?.data?.job_ids) &&
        (result as any).data.job_ids[0]) ||
    ...
    await updatePlanWithJobDetails(
      planId,
      jobId,
      ipfsLink,
      fid === undefined ? null : fid
    );
    ...
    if (error && typeof error === "object") {
      ...
      if (
        errorObj.errorCode === "BALANCE_ERROR" ||
        errorObj.errorType === "BALANCE_ERROR" ||
        errorObj.error === "Failed to deposit ETH balance"
      ) {
        console.log(
          "[TriggerX] Detected BALANCE_ERROR, extracting ethAmount..."
        );
...
export async function updatePlanWithJobDetails(
  ...
  try {
    ...
    const response = await fetch(
      `${DCA_API_URL}/api/dca/plans/${planId}/details`,
      {
        method: "PUT",
        headers: {
...
export async function getSignerFromWallet(): Promise<any> {
  try {
    ...
    if (typeof window === "undefined") {
      throw new Error(
        "getSignerFromWallet can only be called in browser environment"
      );
    }
...
export async function minimalTriggerXExample() {
  const { TriggerXClient, createJob, JobType, ArgType } = await import(
    "sdk-triggerx"
  );
  ...
  const client = new TriggerXClient(
    process.env.NEXT_PUBLIC_TRIGGERX_API_KEY || ""
  );
 ...
 */
export async function deleteTriggerXJobForPlan(
  jobId: string,
  signer: any,
      ...
      get(target, prop) {
        ...
        if (prop === "sendTransaction") {
          return async function (...args: any[]) {
            try {
              const result = await (original as (...args: any[]) => any).apply(
                target,
                args
              );
              return result;
    ...
    try {
      ...
      if (result && typeof result === "object") {
        ...
        if (result.success === false) {
          const errorMsg = (result.error || resultAny.message || "")
            .toString()
            .toLowerCase();
          const isUserRejection =
      ...
      const code =
        apiError?.code || apiError?.error?.code || apiError?.info?.error?.code;
...
export async function fetchETHBalanceForUser(userAddress: `0x${string}`, chainId: string = "42161") {
  const balance = await arbitrumClient.getBalance({
    address: userAddress,
  });
  return formatEther(balance); // ETH string
}
